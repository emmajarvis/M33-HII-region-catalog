from pathlib import Path
import papermill as pm

import logging
import sys

# python -Xfrozen_modules=off your_script.py

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",
    handlers=[logging.StreamHandler(sys.stdout)],
)
logging.getLogger("papermill").setLevel(logging.INFO)

FIELDS = [
    "NW", 
    "NE", 
    "SW", 
    "SE",
    "F5", 
    "F6", 
    "F7", 
    "F8", 
    "F9"
]

# Field-specific notebooks
FIELD_NOTEBOOKS = [
    "2a_make_laplace_maps.ipynb",
    "2b_make_threshold_maps.ipynb",
    "3_finalize_peak_detection.ipynb",
    "4_ZoI.ipynb",
    "5_domains.ipynb",
    "6_flux_catalog_individual_field.ipynb",
]

# Notebooks to run once after all fields are done
FINAL_NOTEBOOKS = [
    "7_create_total_M33_catalog.ipynb",
    # "8_catalog_plots.ipynb",
]

OUT_ROOT = Path("executed")
OUT_ROOT.mkdir(exist_ok=True)

failures = []  # (field, notebook, params, error_message)

all_fields_completed = True

for field in FIELDS:
    print(f"\n=== Running pipeline for {field} ===\n")
    field_out = OUT_ROOT / field
    field_out.mkdir(parents=True, exist_ok=True)

    field_failed = False

    for nb in FIELD_NOTEBOOKS:
        if field_failed:
            break

        in_path = Path(nb)

        if not in_path.exists():
            msg = f"Notebook file not found: {in_path}"
            print(msg)
            failures.append((field, nb, {}, msg))
            field_failed = True
            break

        if in_path.name == "2a_make_laplace_maps.ipynb":
            for sizedet in [1, 10]:
                out_path = field_out / f"{in_path.stem}__{field}__sizedet{sizedet}.ipynb"
                params = {"field": field, "sizedet": sizedet}

                print(f"\n*****\nRunning {in_path} field={field} sizedet={sizedet}\n*****\n")

                try:
                    pm.execute_notebook(
                        input_path=str(in_path),
                        output_path=str(out_path),
                        parameters=params,
                        log_output=False,
                        progress_bar=True,
                    )
                except Exception as e:
                    msg = str(e).strip() or "(no message)"
                    print(f"FAILED field={field} notebook={nb} sizedet={sizedet}")
                    print("Skipping remaining notebooks for this field :(")
                    failures.append((field, nb, params, msg))
                    field_failed = True
                    break

        else:
            out_path = field_out / f"{in_path.stem}__{field}.ipynb"
            params = {"field": field}

            print(f"\n*****\nRunning {in_path} field={field}\n*****\n")

            try:
                pm.execute_notebook(
                    input_path=str(in_path),
                    output_path=str(out_path),
                    parameters=params,
                    log_output=False,
                    progress_bar=True,
                )
            except Exception as e:
                msg = str(e).strip() or "(no message)"
                print(f"\nFAILED field={field} notebook={nb}")
                print("Skipping remaining notebooks for this field :(")
                failures.append((field, nb, params, msg))
                field_failed = True
                break

    if field_failed:
        all_fields_completed = False
    else:
        print(f"\n*******\nCompleted field {field}. Yippee\n*******\n")

print("\nAll fields attempted.")

# Only run notebooks 7/8 if every field completed successfully
if all_fields_completed:
    print("\n=== Running final notebooks after all fields completed ===\n")

    final_out = OUT_ROOT / "final"
    final_out.mkdir(parents=True, exist_ok=True)

    for nb in FINAL_NOTEBOOKS:
        in_path = Path(nb)

        if not in_path.exists():
            msg = f"Notebook file not found: {in_path}"
            print(msg)
            failures.append(("ALL_FIELDS", nb, {}, msg))
            break

        out_path = final_out / f"{in_path.stem}__final.ipynb"
        params = {}

        print(f"\n*****\nRunning final notebook {in_path}\n*****\n")

        try:
            pm.execute_notebook(
                input_path=str(in_path),
                output_path=str(out_path),
                parameters=params,
                log_output=False,
                progress_bar=True,
            )
        except Exception as e:
            msg = str(e).strip() or "(no message)"
            print(f"\nFAILED final notebook={nb}")
            failures.append(("ALL_FIELDS", nb, params, msg))
            break
else:
    print("\nSkipping final notebooks because at least one field failed.")

if failures:
    print("\n--- Failure summary ---")
    for i, (field, nb, params, msg) in enumerate(failures, start=1):
        print(f"{i:02d}) field={field} notebook={nb} params={params}")
        print(f"    error={msg}")
else:
    print("\nNo failures. Woohoo!!!!!")